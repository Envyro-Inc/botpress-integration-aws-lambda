...
import { listLayersInputSchema, listLayersOutputSchema } from '../misc/custom-schemas';
...
export const listLayers: Implementation['actions']['listLayers'] = async ({ ctx, logger, input }) => {
  const validatedInput = listLayersInputSchema.parse(input);
  ...
  try {
    const { compatibleRuntime, limit } = validatedInput;
    ...
    result = await lambdaClient.listLayers(runtime, limit);

    logger.forBot().debug(`Successful - List Layers`);
    ...
    logger.forBot().debug(`'List Layers' exception ${JSON.stringify(error)}`);
    throw error;
  }
};
